      ...
      <div>
        <div class="card">
          ...
          <div class="card-body">
            ...
            <div class="form-field"><label>Date</label><input id="booking-date" type="date" class="rx-input" value="2026-04-15"/></div>
            <div class="form-field"><label>Consultation Type</label>
              <select id="booking-type" class="rx-input"><option value="video">Video Consultation</option><option value="in_person">In-Person</option><option value="chat">Chat Consultation</option></select>
            </div>
            ...
            <div class="form-field"><label>Reason for visit</label><input id="booking-reason" class="rx-input" placeholder="Briefly describe your symptoms…"/></div>
            <div id="booking-message" class="login-error" style="display:none;margin-bottom:12px"></div>
            <button class="btn-sm" style="width:100%;" onclick="confirmBooking()">Confirm Booking</button>
          </div>
        </div>
      </div>
    </div>`,
...
const AD = {
  ...
  'ad-users': () => `
    ...
    <div class="card">
      <div class="card-header"><div class="card-title">Add New Patient</div></div>
      <div class="card-body">
        <div class="form-field">
          <label>Name</label>
          <input id="add-patient-name" type="text" placeholder="John Doe" />
        </div>
        <div class="form-field">
          <label>Date of Birth</label>
          <input id="add-patient-dob" type="date" />
        </div>
        <div class="form-field">
          <label>Gender</label>
        ...
        <div class="form-field">
          <label>Age</label>
          <input id="add-patient-age" type="number" min="0" placeholder="30" />
        </div>
        <div class="form-field">
          <label>Blood Group</label>
          <select id="add-patient-blood-group">
        ...
        <div class="form-field">
          ...
          <input id="add-patient-height" type="number" min="0" placeholder="170" />
        </div>
        <div class="form-field">
          <label>Weight (kg)</label>
          <input id="add-patient-weight" type="number" min="0" placeholder="70" />
        </div>
        <div class="form-field">
          <label>Phone</label>
          <input id="add-patient-phone" type="text" placeholder="+91 9876543210" />
        </div>
        <div class="form-field">
          <label>Address</label>
          <textarea id="add-patient-address" placeholder="123 Main St, City"></textarea>
        </div>
        <div class="form-field">
          <label>City</label>
          <input id="add-patient-city" type="text" placeholder="Mumbai" />
        </div>
        <div class="form-field">
          <label>State</label>
          <input id="add-patient-state" type="text" placeholder="Maharashtra" />
        </div>
        <div class="form-field">
          <label>Allergies</label>
          <input id="add-patient-allergies" type="text" placeholder="Peanuts, Dust" />
        </div>
        <div class="form-field">
          <label>Condition Note</label>
          <textarea id="add-patient-condition" placeholder="Primary health condition"></textarea>
        </div>
        <div id="add-patient-message" class="login-error" style="display:none;margin-bottom:12px"></div>
        <button class="btn-sm" onclick="submitNewPatient()">Add Patient</button>
      </div>
    </div>
    <div class="card">
      <div class="card-header">
  ...
  'ad-doctors': () => `
    ...
    <div class="card">
      ...
      <div class="card-body">
        <div class="form-field">
          <label>Name</label>
          <input id="add-doctor-name" type="text" placeholder="Dr. Anjali Kapoor" />
        </div>
        <div class="form-field">
          <label>Specialisation</label>
          <input id="add-doctor-speciality" type="text" placeholder="Cardiology" />
        </div>
        <div class="form-field">
          <label>License Number</label>
          <input id="add-doctor-license" type="text" placeholder="MH-12345" />
        </div>
        <div class="form-field">
          <label>Experience (years)</label>
          <input id="add-doctor-experience" type="number" min="0" placeholder="5" />
        </div>
        <div id="add-doctor-message" class="login-error" style="display:none;margin-bottom:12px"></div>
        <button class="btn-sm" onclick="submitNewDoctor()">Add Doctor</button>
      </div>
    </div>
    <div class="card">
      <div class="card-header"><div class="card-title">All Doctors</div></div>
      <div class="card-body" style="padding:0;">
        <table class="data-table">
  ...
  'ad-inventory': () => `<div class="section-title">Inventory</div>
    <div class="card"><div class="card-header"><div class="card-title">Add New Stock Item</div></div>
      <div class="card-body">
        <div class="form-field"><label>Medicine Name</label><input id="add-inventory-name" class="rx-input" type="text" placeholder="Amoxicillin 500mg"/></div>
        <div class="form-field"><label>Quantity</label><input id="add-inventory-qty" class="rx-input" type="number" min="0" placeholder="200"/></div>
        <div class="form-field"><label>Minimum Stock</label><input id="add-inventory-minqty" class="rx-input" type="number" min="0" placeholder="20"/></div>
        <div class="form-field"><label>Price</label><input id="add-inventory-price" class="rx-input" type="number" min="0" step="0.01" placeholder="120"/></div>
        <div class="form-field"><label>Expiry Date</label><input id="add-inventory-expiry" class="rx-input" type="date"/></div>
        <div id="add-inventory-message" class="login-error" style="display:none;margin-bottom:12px"></div>
        <button class="btn-sm" onclick="submitNewInventory()">Add Inventory Item</button>
      </div>
    </div>
...
const PH = {
      ...
      <div class="card">
        ...
        <div class="card-body">
          <table class="data-table">
            ...
            <tbody>${DATA.orders.slice(0,5).map(o=>`<tr data-order-id="${o.id}"><td style="font-weight:600;color:var(--blue)">${o.id}</td><td>${o.patient}</td><td style="max-width:160px">${o.items}</td><td style="font-weight:600">${o.amt}</td>
            <td><span class="badge ${o.status} order-status-badge">${o.label}</span></td>
            <td><button class="btn-sm" style="font-size:11px;padding:5px 10px;" onclick="event.stopPropagation();updatePhOrderStatus('${o.id}','Dispatched')">Update</button></td></tr>`).join('')}
            </tbody>
          </table>
        </div>
      </div>
  ...
  'ph-catalog': () => `<div class="section-title">Medicine Catalog</div>
    <div class="med-grid">${DATA.inventory.map(i=>`
      <div class="med-card">
        <div class="med-icon">💊</div>
        <div class="med-name">${i.name}</div>
        <div class="med-brand">SKU: ${i.sku}</div>
        <div class="med-price">${i.price}</div>
        <div class="med-stock ${i.qty < i.min ? 'b-cancel' : 'b-live'}">${i.qty} in stock</div>
  ...
  'ph-orders': () => `<div class="section-title">Order Management</div>
    ...
    <div class="card-body" style="padding:0;"><table class="data-table"><thead><tr><th>Order ID</th><th>Patient</th><th>Items</th><th>Amount</th><th>Date</th><th>Status</th><th>Action</th></tr></thead>
    <tbody>${DATA.orders.map(o=>`<tr data-order-id="${o.id}"><td style="font-weight:600;color:var(--blue)">${o.id}</td><td>${o.patient}</td><td>${o.items}</td><td style="font-weight:600">${o.amt}</td><td style="color:var(--t2)">${o.date}</td>
    <td><span class="badge ${o.status} order-status-badge">${o.label}</span></td>
    <td><select class="rx-input" style="width:100px;padding:4px 8px;font-size:11px;" onchange="updatePhOrderStatus('${o.id}', this.value, this)">
      <option${o.label==='Rx Pending'?' selected':''}>Rx Pending</option>
      <option${o.label==='Packing'?' selected':''}>Packing</option>
      <option${o.label==='Dispatched'?' selected':''}>Dispatched</option>
      <option${o.label==='Delivered'?' selected':''}>Delivered</option>
      <option${o.label==='Cancelled'?' selected':''}>Cancelled</option>
    </select></td></tr>`).join('')}
    </tbody></table></div></div>`,
  ...
  'ph-rx-verify': () => `<div class="section-title">Prescription Verification</div>
    <div class="card"><div class="card-body">
      ${[['PRX-2201','Priya Ramesh','Dr. S. Mehta','Amoxicillin 500mg, Paracetamol 650mg','b-wait','Pending'],['PRX-2198','Manoj Kumar','Dr. P. Singh','Atorvastatin 20mg, Metformin 850mg','b-wait','Pending'],['PRX-2195','Neha Desai','Dr. K. Verma','Paediatric Syrup, Vitamin C','b-live','Verified']].map(([id,p,doc,meds,bs,bl])=>{
        const status = window.PH_RX_STATUS && window.PH_RX_STATUS[id] ? window.PH_RX_STATUS[id] : bl;
        const badgeClass = status === 'Verified' ? 'b-live' : bs;
        return `
        <div class="ph-rx-card" data-rx-id="${id}" style="border:1px solid var(--border);border-radius:var(--r-md);padding:14px;margin-bottom:10px;">
          <div style="display:flex;justify-content:space-between;align-items:center;margin-bottom:8px;">
            <div style="font-weight:600;color:var(--blue)">${id}</div><span class="badge ${badgeClass} ph-rx-badge">${status}</span>
          </div>
          ...
          <div class="ph-rx-action">
            ${status === 'Pending' ? `<div style="display:flex;gap:8px;"><button class="btn-sm" style="background:var(--green);" onclick="verifyPrescription('${id}')">✓ Verify</button> <button class="btn-sm" style="background:var(--red-l);color:var(--red);" onclick="rejectPrescription('${id}')">✕ Reject</button></div>` : `<div class="badge b-live">✅ Verified — ready for dispensing</div>`}
          </div>
        </div>`;
      }).join('')}
    </div></div>`,
